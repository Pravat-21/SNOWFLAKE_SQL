SELECT * FROM PP_PRACTICE.PUBLIC.PP_CUSTOMER_REGION_DETAILS;
SELECT GET_DDL('TABLE','PP_PRACTICE.PUBLIC.PP_CUSTOMER_REGION_DETAILS');
----------------------------------------------------------------------------------------------
...
create or replace procedure demo_procedure1()
returns string
language sql
as
$$
begin
create or replace table demo_procedure1 as 
select age,job,education from bank_details;
end;
$$;

call demo_procedure1();

select * from demo_procedure;

SELECT * FROM BANK_DETAILS;
------------------------------------------------------------------------------

-- CREATE A STORE PROCEDURE FOR AVERAGE BALANCE

CREATE OR REPLACE PROCEDURE AVG_BALANCE()
RETURNS NUMBER(10,3)
LANGUAGE SQL
AS
$$
...
BEGIN
SELECT AVG(BALANCE) INTO AVG_BALANCE FROM BANK_DETAILS;
RETURN AVG_BALANCE;
END;
$$;
CALL AVG_BALANCE();
...
CREATE OR REPLACE PROCEDURE GET_AVG_BALANCE()
RETURNS NUMBER(10,3)
--LANGUAGE SQL
AS
$$
...
-- MARITAL-BALANCE TABLE SHOW
CREATE OR REPLACE PROCEDURE MARITAL_BAL()
RETURNS TABLE(A VARCHAR(20),B NUMBER)
LANGUAGE SQL
AS
$$
DECLARE
RESULT RESULTSET;
BEGIN
RESULT:=(SELECT MARITAL,SUM(BALANCE)AS TOTAL_BALANCE FROM BANK_DETAILS GROUP BY 1 ORDER BY 2);
RETURN TABLE(RESULT);
END;
$$;
SELECT MARITAL,SUM(BALANCE)AS TOTAL_BALANCE FROM BANK_DETAILS GROUP BY 1 ORDER BY 2;

CALL MARITAL_BAL();

------------------------------------------------------------------------------------------------
--CREATE A STORE PROCEDURE TO STORE THE PREVIOUS TABLE
CREATE OR REPLACE PROCEDURE MARITAL_BAL_TABLE()
RETURNS STRING
LANGUAGE SQL
AS
$$
BEGIN
CREATE OR REPLACE TABLE MAT_BAL_TABLE AS 
(SELECT MARITAL,SUM(BALANCE)AS TOTAL_BALANCE FROM BANK_DETAILS GROUP BY 1 ORDER BY 2);
END;
$$;
CALL MARITAL_BAL_TABLE();

SELECT * FROM BANK_DETAILS;
SELECT * FROM PP_PRACTICE.PUBLIC.MAT_BAL_TABLE;
--------------------------------------------------------------------------------------------------------------------
--CREATE A PARAMETRIC STORE PROCEDURE WHERE PASS MARITAL AND EDUCATION ,BASED ON THAT FIND TOTAL BALANCE
CREATE OR REPLACE PROCEDURE PARA_BALANCE( A VARCHAR(30),B VARCHAR(30))
RETURNS NUMBER
LANGUAGE SQL
AS
$$
...
BAL NUMBER;
BEGIN
SELECT SUM(BALANCE) INTO BAL FROM BANK_DETAILS WHERE MARITAL=:A AND EDUCATION =:B ;
RETURN BAL;
END;
$$;

CALL PARA_BALANCE('single','unknown');

SELECT SUM(BALANCE) as BAL FROM BANK_DETAILS WHERE MARITAL='single' AND EDUCATION ='unknown';

----------------------------------------------------------------------------------------------------
select * from bank_details;
-- CREATE A PROCEDURE BASED ON JOB,MARITAL,EDUCATION TO FIND AVERAGE AND TOTAL BALANCE
CREATE OR REPLACE PROCEDURE JME_DETAILS(A VARCHAR(30),B VARCHAR(30),C VARCHAR(30))
RETURNS TABLE(A NUMBER,B NUMBER)
LANGUAGE SQL
AS
$$
DECLARE
RES RESULTSET;
BEGIN
RES:=(SELECT SUM(BALANCE) AS TOTAL_BAL,AVG(BALANCE)AS AVG_BAL FROM BANK_DETAILS WHERE JOB=:A AND MARITAL=:B AND EDUCATION=:C);
RETURN TABLE(RES);
END;
$$;

CALL JME_DETAILS('blue-collar','married','secondary');

SELECT SUM(BALANCE) AS TOTAL_BAL,AVG(BALANCE)AS AVG_BAL FROM BANK_DETAILS WHERE JOB='blue-collar' AND MARITAL='married' AND EDUCATION='secondary';